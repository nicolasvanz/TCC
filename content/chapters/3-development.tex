...
\begin{description}[leftmargin=*,labelwidth=!,labelindent=0pt]
    ...
    \end{description}
    De acordo com o estado atual, são configurados os \buffers para o envio/recebimento de dados (respectivamente nas \tasks 6 e 7), os limites inferior e superior do \myloop que percorre o \buffer (na \task 5) e uma função de condição que verifica se elemento em dado índice do \buffer atual precisa ser enviado.
    \item[5.] Esta \task é responsável por atualizar o índice atual do \buffer configurado na \task 4 e indicar se a estrutura foi completamente percorrida ou não. Caso o limite superior, também configurado pela \task 4, foi atingido, a \task 4 á ativa, atualizando o estado atual. Caso contrário, um novo índice é considerado para o envio/recebimento de dados.
    \item[6.] Esta \task é ativa desde que o \cluster exerça o papel de remetente na migração. A \task é responsável por verificar se a posição atual na estrutura atual precisa ser enviada. Em caso positivo, a \task 7 é ativada para o envio de dados. Caso contrário, a \task 5 é ativada para atualizar o índice para o próximo elemento do \buffer.
    ...
    \item[9.] Esta \task é ativa desde que o \cluster exerça o papel de destinatario na migração. A \task é responsável por verificar se a posição atual na estrutura atual precisa ser recebida. Em caso positivo, a \task 10 é ativada para o recebimento dos dados. Caso contrário, a \task 5 é ativada para atualizar o índice para o próximo elemento do \buffer.
